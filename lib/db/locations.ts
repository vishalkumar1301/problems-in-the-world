...
export async function getLocationById(id: number): Promise<Location | null> {
  const [rows] = await pool.query<RowDataPacket[]>('SELECT * FROM locations WHERE location_id = ?', [id]);
  return rows[0] as Location | null;
}

export async function addLocation(location: Omit<Location, 'location_id'>): Promise<number> {
...
export async function updateLocation(id: number, location: Partial<Location>): Promise<boolean> {
  const { name, type, parent_location_id } = location;
  const [result] = await pool.query<ResultSetHeader>(
...
export async function deleteLocation(id: number): Promise<boolean> {
  const [result] = await pool.query<ResultSetHeader>('DELETE FROM locations WHERE location_id = ?', [id]);
  return result.affectedRows > 0;
}